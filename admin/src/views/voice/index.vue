      ...
      <el-form label-width="100px">
        ...
        <div class="flex gap-2">
          ...
          <el-form-item label="采样率">
            <el-input-number
              v-model="paramDialog.sample_rate"
              :step="1000"
              :min="8000"
              :max="48000"
            />
  ...
  async function fetchList() {
    ...
    try {
      ...
      if (Number.isFinite(totalCandidate) && totalCandidate > 0) {
        total.value = totalCandidate;
      } else {
        // 无总数字段时，回退为“已知最小总数”估算，保证分页可用
        const est =
          (Number(listQuery.page_index) || 0) * (Number(listQuery.page_size) || 0) + arr.length;
        total.value = est;
      }
      uiPage.value = (Number(listQuery.page_index) || 0) + 1;
  ...
  async function openSetParams(row: any) {
    ...
    try {
      ...
      if (data && typeof data === 'object') {
        ...
        if (typeof data.sample_rate === 'number') paramDialog.sample_rate = data.sample_rate;
    ...
    try {
      await voiceApi.setParams({
        ...
        params: {
          ...
          sample_rate: paramDialog.sample_rate,
