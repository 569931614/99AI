...
import { fetchGroupMembersAPI } from '@/api/group'
...
const onConversation = async ({
  ...
  const handleGroupChatResponse = async () => {
    try {
      ...
      for (let i = 0; i < groupMembers.length; i++) {
        ...
        console.log(`[群聊] 第 ${i + 1} 个角色开始发言: ${memberName}`)
  ...
  const getGroupMembers = async () => {
    try {
      console.log('[群聊] 获取成员，groupId:', activeGroupId.value)
      const response = await fetchGroupMembersAPI({ groupId: activeGroupId.value })
      console.log('[群聊] 成员响应:', response)

      // API返回格式：{ code: 200, data: [...], success: true }
      const members = (response as any)?.data || (response as any)?.members || []
      console.log('[群聊] 解析到的成员列表:', members)

      return members
    } catch (error) {
      console.error('[群聊] 获取成员失败:', error)
      return []
    }
  }
  ...
  const handleGroupChatCompleteResponse = (
    ...
    try {
      ...
      // 找到对应成员的聊天记录并更新
      // 从后往前找，找到第一个 memberIndex 匹配且 loading 为 true 的记录
      let found = false
      for (let i = dataSources.value.length - 1; i >= 0; i--) {
        const chat = dataSources.value[i] as any
        if (chat.memberIndex === memberIndex && chat.loading) {
    ...
    try {
      ...
      await fetchChatAPIProcess({
        ...
        onDownloadProgress: ({ event }) => {
          // 使用新的fetch流式处理
          // responseText 是当前这次回调收到的chunk（可能包含多个JSON行）
          ...
          try {
            ...
            console.log('[流式响应] 收到chunk，解析到', jsonLines.length, '行JSON')

            jsonLines.forEach((line: string, index: number) => {
              try {
                const jsonObj = JSON.parse(line)

                console.log(`[流式响应] 解析JSON行 ${index + 1}/${jsonLines.length}:`, {
                  hasContent: !!jsonObj.content,
                  contentText: jsonObj.content ? jsonObj.content[0]?.text.substring(0, 50) : null,
                  hasReasoningContent: !!jsonObj.reasoning_content,
                ...
                if (jsonObj.content) {
                  ...
                  if (completeText.length >= fullText.length) {
                    if (isCacheEnabled) {
                      ...
                      if (newText) {
                        // 判断是否是第一个字符
                        const isFirstContent = fullText.length === newText.length
                    ...
                    console.warn('[流式响应] 收到的文本长度小于之前的长度，忽略此次更新')
                  }
                }
      ...
      } else if (!displayedText || displayedText.length === 0) {
        // 只有在 displayedText 为空时才使用 fullText
        displayedText = fullText
      }

      if (!displayedReasoningText || displayedReasoningText.length === 0) {
        displayedReasoningText = fullReasoningText
      }

      console.log(
        '[流式响应] finally块 - fullText长度:',
        fullText.length,
        'displayedText长度:',
        displayedText.length
      )
