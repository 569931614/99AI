...
import MemberList from '@/views/chat/components/Group/MemberList.vue'
import {
  Brightness,
  CheckOne,
  Close,
...
<template>
        ...
        <div class="flex justify-between items-center h-full w-full">
          ...
          <div class="flex items-center">
            ...
            <div v-if="!externalLinkActive && !isPreviewerVisible" class="relative group mx-1">
              ...
              <div v-if="!isMobile" class="tooltip tooltip-bottom">群聊成员</div>
            </div>
  ...
  <!-- 成员抽屉面板 -->
  <div v-if="showMembers" class="fixed inset-0 z-40">
    <div class="absolute inset-0 bg-black/20" @click="toggleMembers" />
    <div
      class="absolute right-0 top-0 bottom-0 w-[90vw] sm:w-[420px] md:w-[560px] lg:w-[720px] bg-white dark:bg-neutral-900 shadow-xl border-l dark:border-neutral-800 flex flex-col"
    >
      <div
        class="flex items-center justify-between p-3 border-b dark:border-neutral-800 flex-shrink-0"
      >
        <div class="text-sm font-semibold">群聊成员</div>
        <button type="button" class="btn-icon btn-sm" @click="toggleMembers" aria-label="关闭">
          <Close size="16" />
        </button>
      </div>
